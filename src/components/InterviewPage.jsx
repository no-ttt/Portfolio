...
function SectionHeader({ kicker, title, desc, light }) {
  return (
    <div style={{ marginBottom: 34 }}>
      <div style={{ fontFamily: 'var(--font-jp)', fontWeight: 700, fontSize: '0.82rem', letterSpacing: '0.16em', color: light ? 'var(--gold)' : 'var(--red)', textTransform: 'uppercase', marginBottom: 12 }}>
        {kicker}
      </div>
      <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 800, fontSize: 'clamp(2rem,5vw,3.4rem)', textTransform: 'uppercase', letterSpacing: '-0.02em', margin: '0 0 14px', color: light ? 'var(--cream)' : 'var(--ink)' }}>
        {title}
      </h2>
      {desc && <p style={{ fontWeight: 500, fontSize: '1.05rem', lineHeight: 1.55, color: light ? 'rgba(255,245,230,0.85)' : 'var(--text-body)', margin: 0, maxWidth: '52ch' }}>{desc}</p>}
    </div>
  );
}
...
export function InterviewPage() {
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      {/* 接案 */}
      <section style={{ padding: 'clamp(48px,6vw,88px) clamp(16px,4vw,48px)', background: 'var(--cream)', borderBottom: '3.5px solid var(--ink)' }}>
        <div className="pl-container">
          <SectionHeader
            kicker="個人接案 · FREELANCE"
            title="接案作品"
            desc="從品牌官網到爬蟲自動化 — 點選類別篩選，或點開專案看完整介紹。"
          />
      ...
      <section style={{ background: 'var(--cream)', borderBottom: '3.5px solid var(--ink)', padding: 'clamp(48px,6vw,88px) clamp(16px,4vw,48px)' }}>
        <div className="pl-container">
          <SectionHeader
            kicker="個人專案 · SIDE PROJECTS"
            title="Side Projects"
            desc="學生時期開發 — 從想法到落地，完整從 0 到 1。"
          />
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,280px),1fr))', gap: 24 }}>
